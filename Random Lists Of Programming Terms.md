...
Unread : Angular Observables https://medium.com/@luukgruijs/understanding-creat…
...
Do this: Build an API with HAPI/Mongo/Node/GraphQL/Swagger https://medium.freecodecamp.org/how-to-setup-a-pow…
...
CHILD : 
    ...
    CREATE NEW FOLDER UNDER 'THEMES' FOLDER
        CALL CHILD FOLDER SAME NAME AS PROJECT (NOTE : SAME LEVEL AS ROOT PARENT THEME)
    WILL LOOK TO ROOT THEME FIRST
        THEN CHILD THEME (CHECK OUT WORDPRESS HIERARCHY!)

    LIST OF FILES IT CHECKS IN WHAT ORDER

    CHILD
        ADD INTO THIS ANY CHILD FILES YOU WANT

            FUNCTIONS.PHP IS KEY
            STYLE.CSS IS KEY

                (GO TO PARENT AND COPY STYLE FROM IT)

            FUNCTIONS.PHP IS BIGGEST FILE
                DELETE EVERYTHING EXCEPT ENQEUE FUNCTION AT TOP WHICH DICTATES ORDER OF PULLING 

           HEADER.PHP : COPY TO CHILD THEN EDIT AND STRIP OUT AS MUCH AS YOU CAN

          PAGE.PHP : TEMPLATE FOR DISPLAYING PAGE

                 CAN COPY AND THEN RENAME …

    BACK IN WORDPRESS ADMIN …

        PAGES => NEW PAGE (CAN CHOOSE A TEMPLATE).

                    CAN NOW CHOOSE A PAGE !!!!

    BACK IN ROOT FILES

    FUNCTIONS.PHP

        IMAGES … WHEN UPLOAD … TEMPLATE PRE-DEFINES SIZES WHICH ARE AVAILABLE

        SHORTCODES … 

    WIDGET = EG ADVERTS = SIDEBAR</name-of-branch> </pre>
