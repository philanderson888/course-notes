...
function displaySummaryData(data) {
    ...
    console.log(' ');
    console.log('display summary data ...');
    ...
    const giftAidGivingTableBody = document.querySelector('#gift-aid-giving-table tbody');
    giftAidGivingTableBody.innerHTML = '';
    let giftAidTotal = 0;

    console.log(' ');
    console.log('gift aid giving table ...');

    mergedRenamedFilteredSortedCharityCategories.forEach(([recipient, roundedTotal]) => {
        if (recipient === 'Tithe' || recipient === 'Offering' || recipient === 'GFA' || recipient === 'CFAN' || recipient === 'Burn' || recipient === 'God TV' || recipient === 'Wikipedia') {
            console.log(recipient, roundedTotal);
            const row = document.createElement('tr');
            row.innerHTML = `
                <td>${recipient}</td>
                <td>£${roundedTotal.toFixed(0)}</td>
            `;
            giftAidGivingTableBody.appendChild(row);
            giftAidTotal += roundedTotal;
        }
    });


    const nonGiftAidGivingTableBody = document.querySelector('#non-gift-aid-giving-table tbody');
    nonGiftAidGivingTableBody.innerHTML = '';
    let nonGiftAidTotal = 0;

    console.log(' ');
    console.log('non gift aid giving table ...');

    mergedRenamedFilteredSortedCharityCategories.forEach(([recipient, roundedTotal]) => {
    if (recipient === 'Raul' || recipient === 'Jonathan' || recipient === 'One-Off' || recipient === 'Orphan School' || recipient === 'Learning School' || recipient === 'Saba' || recipient === 'Shakaib' 
            || recipient === 'Saima' || recipient === 'Daniyal' || recipient === 'Zeeshan' 
            || recipient === 'Shaloom' || recipient === 'Shumaila' || recipient === 'Sumaira' || recipient === 'Hasroon' 
            || recipient === 'Zeerish' || recipient === 'Sehrish' || recipient === 'Rifqa' || recipient === 'Gulraiz') {
            console.log(recipient, roundedTotal);
            ...
            row.innerHTML = `
                ...
                <td>£${roundedTotal.toFixed(0)}</td>
            `;
            nonGiftAidGivingTableBody.appendChild(row);
            nonGiftAidTotal += roundedTotal;
        }
    });
    ...
    const totalGiftAidGiving = giftAidTotal;
    document.getElementById('total-gift-aid-giving').textContent = `Total Gift Aid Giving - £${totalGiftAidGiving.toFixed(0)}`;

    const totalNonGiftAidGiving = nonGiftAidTotal;
    document.getElementById('total-non-gift-aid-giving').textContent = `Total Non Gift Aid Giving - £${totalNonGiftAidGiving.toFixed(0)}`;
